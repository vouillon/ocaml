...
let hint_table = (Hashtbl.create 253 : (int, performance_hint) Hashtbl.t)
...
let dump_obj ic =
  ...
  if cu.cu_hint > 0 then begin
    ...
    let evl = (Compression.input_value ic : (int * performance_hint) list) in
    record_hints 0 evl
  end;
  ...
  begin
    ...
    | (_ : int) ->
        ...
        for _i = 1 to num_hintlists do
          let orig = input_binary_int ic in
          let hints = (Compression.input_value ic : (int * performance_hint) list) in
