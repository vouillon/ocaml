      ...
      let print_value_kind (k : Lambda.value_kind) =
        printf
          (match k with
           ...
           | Pintval -> "int")
      in
      List.iter (fun  {params; return; inline; specialise; is_a_functor} ->
          printf " (";
          List.iter (fun k -> print_value_kind k; printf " -> ") params;
          print_value_kind return;
          (match inline with
           | Always_inline -> printf " inline"
           | Never_inline -> printf " inline:never"
           | Hint_inline -> printf " inline:hint"
