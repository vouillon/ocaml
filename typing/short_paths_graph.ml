...
module Path = struct
  ...
  let rec equal t1 t2 =
    ...
    | Pextra_ty _, Pident _ -> false
    | Pextra_ty _, Pdot _ -> false
    | Pextra_ty _, Papply _ -> false
    | Pextra_ty (p1, t1), Pextra_ty (p2, t2) ->
      let same_extra =
        match t1, t2 with
        | Pcstr_ty s1, Pcstr_ty s2 -> s1 = s2
        | Pcstr_ty _, Pext_ty -> false
        | Pext_ty, Pcstr_ty _ -> false
...
let hidden_ident id = hidden_name (Ident.name id)
...
and Module_type : sig
  ...
  let base origin id (desc : Desc.Module_type.t option) deprecated =
    let path = Path.Pident id in
    let hidden = hidden_base_definition deprecated id in
    let sort = Sort.Defined in
    let definition =
  ...
  let child md name (desc : Desc.Module_type.t option) deprecated =
    let origin = Module.raw_origin md in
    let sort = Module.raw_sort md in
    let path = Path.Pdot (Module.raw_path md, name) in
    let hidden = hidden_definition deprecated name in
...
and Module : sig
  ...
  let base origin id (desc : Desc.Module.t option) deprecated =
    let path = Path.Pident id in
    let hidden = hidden_base_definition deprecated id in
    let sort = Sort.Defined in
    let definition =
  ...
  let child md name (desc : Desc.Module.t option) deprecated =
    let origin = Module.raw_origin md in
    let sort = Module.raw_sort md in
    let path = Path.Pdot(Module.raw_path md, name) in
    let hidden = hidden_definition deprecated name in
  ...
  let application func arg (desc : Desc.Module.t option) =
